function pi=plot_purple(i,L,N_WT,N_Subs,I,J,s_x,s_y,s_Pij,s_Theta,Coord_WT,Coord_OS,CCS,Bij,n_cab)
%% This function plot the graph of a OWT ECSP result, and return the Handle of the picture
figure;
% colorList=slanCL(type,num);
...
SUB_XCoords = pi.XData(N_Subs);
SUB_YCoords = pi.YData(N_Subs);
% 使用plot函数绘制空心圆
...
hold off;
% plot(SUB_XCoords, SUB_YCoords, 's', 'MarkerSize', 20, 'MarkerEdgeColor', 'k', 'MarkerFaceColor', 'none','LineWidth',1);
Subcolor = [181 40 47]/255;
...
purple_colors = [203 232 238; 210 192 221; 175 140 242; 120 53 132; 89 1 76; 44 22 75] / 255;  % 目前的

% purple_colors = slanCL(1612,[7,6,5,4,3,2]);
for i=1:size(s_x,1)
    for j=1:size(s_x,2)
        if s_x(i,j)==1
...
% WTcolor = slanCL(1618,[3]);
% highlight(pi,N_WT,'Marker','o', 'MarkerSize', 10, 'NodeColor', WTcolor);
